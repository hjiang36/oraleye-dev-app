...
document.addEventListener('DOMContentLoaded', function () {
  ...
  canvas.on('mouse:over', function (e) {
    if (e.target && e.target.type === 'rect') {
      const iconButton = document.getElementById('annotationIcons');
      const iconInfoButton = document.getElementById(
        'annotationInfoIconButton'
      );
      const rect = e.target;
  ...
  function deleteSelectedObject() {
    ...
    const associatedRect =
      document.getElementById('annotationIcons')._associatedRect;
    if (activeObject) {
...
document
  .getElementById('annotationIcons')
...
document
  .getElementById('regionAnnotation')
...
document
  .getElementById('closeInfoSidebar')
...
document
  .getElementById('annotationInfoIconButton')
  .addEventListener('click', function () {
    const infoSidebar = document.getElementById('infoSidebar');
    infoSidebar.style.display = 'block';
    infoSidebar.scrollTop = 0; // Scroll to the top
    canvas.setActiveObject(
      document.getElementById('annotationInfoIconButton')._associatedRect
    );
...
document
  .getElementById('analysisSaveBtn')
  .addEventListener('click', function () {
    // Serialize the canvas to JSON string
    if (canvas) {
      const json = JSON.stringify(
        canvas.toJSON(['annotation', 'hoverFillColor'])
      );
